...
router.post('/survey/skip', requireAuth, requireRole('alumni'), ctrl.skipSurvey);
...
router.post('/career/:entryId(\\d+)/delete', requireAuth, requireRole('alumni'), ctrl.deleteCareerEntry);

// Public-ish (any authenticated user) view of a specific alumnus's profile.
router.get('/:id(\\d+)', requireAuth, ctrl.viewAlumniProfile);
